...
class SM_Complaint {
    public function ReceiveComplaint($connect, $userid, $vehicleid, $complaint) {

        //$complaintid = md5(uniqid(microtime(true),true));
        //$userid = $complaint['userid'];             // userid must exist or database would report error
        $complaintid = self::generateComplaintID(); 
        $userid = $userid;             // userid must exist or database would report error
        $complaint = $complaint;    
        $complainttype = 4;
        $vehicleid = $vehicleid;
        ...
        if(!oci_execute($stid)) {
            Response::show(2403,'User_Complaint-Receieve: Query database error');
            //return false;
        } 
